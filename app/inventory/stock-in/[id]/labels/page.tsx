...
export default async function LabelsPage({params}:{params:Promise<{id:string}>}){
  ...
  return <main className={styles.page}><div className={styles.toolbar}><Link href={`/inventory/stock-in/${stockInId}`}><ArrowLeft size={17}/> Back</Link><div><strong>{labels.length} QR label{labels.length===1?"":"s"}</strong><span>Adjust paper/label size in your printer dialog if needed.</span></div><PrintButton/></div><section className={styles.sheet}>{labels.map((label,index)=><article className={styles.label} key={`${label.batch_number}-${label.copy}-${index}`}><div className={styles.shop}>KL MOTOR SHOP</div><div className={styles.name}>{label.product_name}</div><div className={styles.price}>{peso(Number(label.selling_price))}</div><div className={styles.qr}><StockInQrCode value={label.barcode}/></div><div className={styles.code}>{label.barcode}</div><div className={styles.meta}>{label.product_code} · {label.batch_number}</div></article>)}</section></main>;
}
